...
import { Product, Category, Regiao } from '../models/index.js';
...
export async function listProducts(req, res) {
  try {
    ...
    return res.status(500).json({ error: 'erro ao listar produtos', details: String(err && err.message ? err.message : err) });
  }
}
...
export async function createProduct(req, res) {
  try {
    ...
    return res.status(500).json({ error: 'erro ao criar produto', details: String(err && err.message ? err.message : err) });
  }
}
...
export async function updateProduct(req, res) {
  try {
    ...
    return res.status(500).json({ error: 'erro ao atualizar produto', details: String(err && err.message ? err.message : err) });
  }
}
...
export async function deleteProduct(req, res) {
  try {
    ...
    return res.status(500).json({ error: 'erro ao excluir produto', details: String(err && err.message ? err.message : err) });
  }
}
...
export async function exportProducts(req, res) {
  try {
    ...
    return res.status(500).json({ error: 'erro ao exportar produtos', details: String(err && err.message ? err.message : err) });
  }
}
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (categoriaIdRaw) {
        ...
        const cat = await Category.findOne({ where });
        if (!cat) {
      ...
      let prod = null;
      if (id) {
        prod = await Product.findByPk(id);
      }
      if (prod) {
        if (categoriaId !== null) {
        ...
        const prodCreated = await Product.create({
    ...
    return res.status(500).json({ error: 'erro ao importar produtos', details: String(err && err.message ? err.message : err) });
  }
}

export async function importProductsOld(req, res) {
  try {
    const prods = await Product.findAll({
      include: [{ model: Category, as: 'category' }],
      order: [['created_at', 'DESC']]
    ...
    return res.status(500).json({ error: 'erro ao listar produtos para importação antiga', details: String(err && err.message ? err.message : err) });
  }
}
